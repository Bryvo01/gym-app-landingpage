      ...
      <ProductCard {productFeature} {index}>
        {#if index === 0}
          Experience a beginner exercise <span
            class="text-indigo-600 font-medium">master</span
          > class
        {:else if index === 1}
          Lay the foundations for <span class="text-indigo-600 font-medium"
            >routine</span
          > and consistency
        {:else}
          Workouts specific to your <span class="text-indigo-600 font-medium"
            >environment</span
          >
          and
          <span class="text-indigo-600 font-medium">expertise</span>
        {/if}
      </ProductCard>
    {/each}
  </div>
